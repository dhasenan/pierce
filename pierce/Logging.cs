...
// I'm not sure why, but when using Castle's log4net integration, I get a missing method exception
// regarding a method that shouldn't be called and seems to be defined where we're looking for it.
// It's probably a log4net version dependency issue, but this was easy enough to implement as a
// terrible, hacky workaround.
namespace pierce
{
    public class MyLoggerFactory : AbstractLoggerFactory
    {
        public override Logger Create(string name)
        {
    ...
    public class Log4netLogger : MarshalByRefObject, Logger
    {
        ...
        public Log4netLogger(log4net.Core.ILogger logger, ILoggerFactory factory)
        {
            Logger = logger;
            Factory = factory;
        }
        ...
        internal Log4netLogger(ILog log, ILoggerFactory factory) : this(log.Logger, factory)
        {
        }
        ...
        protected internal ILoggerFactory Factory { get; set; }
