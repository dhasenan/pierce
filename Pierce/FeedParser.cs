...
public class FeedParser
{
    ...
    public FeedParser(ILogger<ReadFeeds> logger)
    {
        _logger = logger;
    }
    ...
    DateTime ParseRfc1123(string v)
    {
        DateTime d = DateTime.UtcNow;
        if (!TryParseRfc1123(v, ref d))
        {
            throw new FormatException("invalid date: " + v);
        }
        return d;
    }
    ...
    bool TryParseRfc1123(string v, ref DateTime date)
    {
        ...
        _logger.LogInformation("bad date found: {0}", v);
        return false;
    }
    ...
    private void ReadAtom(Feed feed, XDocument x)
    {
        ...
        ReadAuthors(xfeed, "author", feed.AuthorInfo.Authors);
        ReadAuthors(xfeed, "contributor", feed.AuthorInfo.Authors);
    ...
    private Article ReadArticle(XElement xentry, DateTime now)
    {
        ...
        Elem(xentry, "author", v => article.AuthorInfo.Authors.Add(new Author { Name = v }));
        Elem(xentry, elements + "creator", v => article.AuthorInfo.Authors.Add(new Author { Name = v }));
        Elem(xentry, "description", v => article.Description = v);
        Elem(xentry, "guid", v => article.UniqueId = v);
        Elem(xentry, "pubDate", v => article.PublishDate = DateTime.Parse(v), v => ParseRfc1123(v));
        ...
        ReadAuthors(xentry, "author", article.AuthorInfo.Authors);
        ReadAuthors(xentry, "contributor", article.AuthorInfo.Authors);
        ...
        if (content != null)
        {
            ...
            if (attr != null)
            {
                if (article.Link == null)
                {
                    ...
                    Uri link;
                    if (Uri.TryCreate(attr.Value, UriKind.Absolute, out link))
                    {
                        article.Link = link;
                    }
                }
