...
var domain = {
  ...
  refreshSubsImpl: function(last, subs, after) {
    $.ajax('/users/subscriptions', {
      ...
      success: function(data) {
        subs.subscriptions.splice(subs.subscriptions.length, 0, data.subscriptions);
        subs.feeds.splice(subs.feeds.length, 0, data.feeds);
        if (data.next) {
  ...
  refreshFeeds: function() {
    ...
    domain.refreshSubs((data) => {
      ...
      domain.feeds = data.feeds;
      domain.feedsByID = new Map();
      domain.feeds.forEach((f) => domain.feedsByID.set(f.id, f));
